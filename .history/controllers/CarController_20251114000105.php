...
class CarController
{
    ...
    public function showUserPage()
    {
        ...
        // Отображаем только основную страницу (без AJAX)
        $this->view->render('userPage.tpl');
    }

    public function listCarsAjax()
    {
        if (!$this->userId) {
            jsonStatus(false, 'User not authorized');
            exit;
        ...
        echo $this->view->fetch('car_list.tpl');
    }
}

// === Обработка запросов ===
$controller = new CarController();
$action = $_POST['status'] ?? ($_GET['action'] ?? 'showUserPage');
...
switch ($action) {
    ...
    case 'delete':
        ...
        break;
    case 'list':
        $controller->listCarsAjax();
        break;
    default:
        // по умолчанию показываем основную страницу (только 1 раз)
